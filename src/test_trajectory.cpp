...
void RunTrajectoryTest(int n, int m, int T, int l, int runs) {
  ...
  std::function<double(const Eigen::VectorXd *)> terminal_cost_f = [Q, q](const Eigen::VectorXd *x) {
    Eigen::MatrixXd val = 0.5 * (*x).transpose() * Q * (*x) + (*x).transpose() * q;
    return val(0, 0);
  };

  std::function<void(const Eigen::VectorXd *, Eigen::VectorXd &)>
      terminal_grad_f = [Q, q](const Eigen::VectorXd *x, Eigen::VectorXd &g) {
    g = Q * (*x) + q;
  };

  std::function<void(const Eigen::VectorXd *, Eigen::MatrixXd &)>
      terminal_hess_f = [Q](const Eigen::VectorXd *, Eigen::MatrixXd &H) {
    H = Q;
  };
...
//  }
  ...
  std::function<void(const Eigen::VectorXd *, const Eigen::VectorXd *, int, Eigen::VectorXd &)>
      running_constraint_f = [](const Eigen::VectorXd *, const Eigen::VectorXd *, int , Eigen::VectorXd &c) { c.setZero(); };

  std::function<void(const Eigen::VectorXd *, const Eigen::VectorXd *, int, Eigen::MatrixXd &)>
      running_constraint_jac_x_f = [](const Eigen::VectorXd *, const Eigen::VectorXd *, int , Eigen::MatrixXd &Jx) { Jx.setZero(); };

  std::function<void(const Eigen::VectorXd *, const Eigen::VectorXd *, int, Eigen::MatrixXd &)>
      running_constraint_jac_u_f = [](const Eigen::VectorXd *, const Eigen::VectorXd *, int , Eigen::MatrixXd &Ju) { Ju.setZero(); };
